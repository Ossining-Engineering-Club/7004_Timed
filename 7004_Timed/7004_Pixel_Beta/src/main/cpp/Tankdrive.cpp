...
using namespace std;
Tankdrive::Tankdrive(int leftPort, int rightPort, int leftPort1, int rightPort1, int leftEncoder1, int leftEncoder2, int rightEncoder1, int rightEncoder2, OECPigeonIMU *pigeonIMU, SmartDashboard *smartDashboard){
    ...
    leftDrive = new frc::Spark(leftPort);
    rightDrive = new frc::Spark(rightPort);
    leftDrive1 = new frc::Spark(leftPort1);
    rightDrive1 = new frc::Spark(rightPort1);
...
void Tankdrive::SetPower(double leftPower, double rightPower){
    ...
    leftDrive1->Set(-throttle*leftPower*(2-STRAIGHT_DRIVE_CORRECTION));
    rightDrive1->Set((throttle*rightPower)*STRAIGHT_DRIVE_CORRECTION);
}
...
void Tankdrive::DriveCurveEncoder(double radius, double degrees, double avgPower){
    ...
    if(leftPower < rightPower){
     ...
     smartDashboard->PutNumber("Left Predicted", leftPower);
     smartDashboard->PutNumber("Right Predicted", rightPower);
    ...
    while(abs(avgDist) < abs(totalDist)){
        ...
        rightEnc = GetRightEncoderDist();
         smartDashboard->PutNumber("leftEncoder", GetLeftEncoderDist());
         smartDashboard->PutNumber("rightEncoder", GetRightEncoderDist());
        ...
        correction = pidController->GetPIDCorrection(error);
         smartDashboard->PutNumber("Left Actual", leftPower + correction);
         smartDashboard->PutNumber("Right Actual", rightPower - correction);
     ...
     smartDashboard->PutNumber("Left Average", leftTotal/count);
     smartDashboard->PutNumber("Right Average", rightTotal/count);
    SetPower(0.0,0.0);
}
